...
## Бекенд
- [x] PostgreSQL + Sequelize ORM + NestJS + TypeScript (strict)
- [x] Конфигурируемые ROUND_DURATION и COOLDOWN_DURATION через .env
- [x] Подсчет очков: 1 тап = 1 очко, каждый 11-й тап = 10 очков
- [x] Тапать можно только в активном раунде
- [x] Роли: выживший (user), admin, nikita — назначаются по имени
- [x] Тапы Никиты не считаются (в статистике нули)
- [x] Консистентность данных при тапах (атомарный increment в БД)
- [x] Stateless бекенд — можно запускать несколько инстансов
- [x] REST API: логин, список раундов, создание раунда (admin), инфо о раунде, тап

## Фронтенд
- [x] React + TypeScript + react-router + Vite
- [x] Страница логина (создание пользователя если нет, ошибка если пароль неверный)
- [x] Список активных/запланированных раундов, кнопка создания (для admin)
- [x] Страница раунда: состояние (cooldown/активен/завершен), гусь для тапов, таймер, очки
- [x] Результаты завершенного раунда: общий счет, победитель, свои очки
